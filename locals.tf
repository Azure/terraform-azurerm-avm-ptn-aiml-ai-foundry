locals {
  base_name_clean        = coalesce(var.base_name, var.name)
  ...
  resource_names = {
    ai_agent_host                   = coalesce(var.resource_names.ai_agent_host, "${local.base_name_clean}-agent-${local.resource_token}")
    ai_foundry_project              = coalesce(var.resource_names.ai_foundry_project, "${local.base_name_clean}-proj-${local.resource_token}")
    ai_foundry_project_display_name = coalesce(var.resource_names.ai_foundry_project_display_name, "AI Foundry Project for ${local.base_name_clean}")
    ai_search                       = coalesce(var.resource_names.ai_search, "srch-${local.base_name_clean}-${local.resource_token}")
    ai_foundry                      = coalesce(var.resource_names.ai_foundry, "ai-${local.base_name_clean}-${local.resource_token}")
    cosmos_db                       = coalesce(var.resource_names.cosmos_db, "cos-${local.base_name_clean}-${local.resource_token}")
    key_vault                       = coalesce(var.resource_names.key_vault, "kv-${local.base_name_clean}-${local.resource_token}")
    storage_account                 = coalesce(var.resource_names.storage_account, "st${local.base_name_clean}${local.resource_token}")
  }
